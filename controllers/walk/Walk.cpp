  ...
  while(1)
  {
    ...
    while (true) 
    {
      ...
      if( cont>=((*mGaitManager->period_time_RL/8)*numero_de_passos) )
      {
        ...
        //cout<<"TRANS1 "<<TRANS1<<" - TRANS2 "<<TRANS2<<" - TRANS3 "<<TRANS3<<endl;
...
void Walk::run2P(bool newFile) {
  ...
  init_RL(); //start the RL's variable (states)
  ...
  *mGaitManager->arm_swing_gain_RL = 1.0;
  ...
  while(1)
  {
    ...
    *mGaitManager->swing_right_left_RL = p2_value[state_p2];
    ...
    //Permanece neste while enquanto passos dado for menor que num_max_passos ou caiu_cont<10
    while (true) 
    {
      checkIfFallen();
	  if(CAIU_CONT==24)
...
{
        //- Calcula distancia e velocidade----------------------------------------
        distancia = sqrt(pow(TRANS1-transIni1,2)+pow(TRANS3-transIni3,2));
